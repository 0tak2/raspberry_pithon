...
def getAllPmData() -> tuple:
    ...
    return (pm10, pm25, stationName)

def getOnePmData(index) -> tuple:
    all_pm10, all_pm25, all_stationName = getAllPmData()
    return (all_pm10[index], all_pm25[index], all_stationName[index])
...
def GPIOLoop(index):
    try:
        while True:
            pm10, pm25, stationName = getOnePmData(index)
            print()
...
def main():
    temp1, temp2, stationName = getAllPmData()
    index = -1
    while True:
